# 创建第一个脚本 process_first800.sh
cat > process_first800.sh << 'EOF'
#!/bin/bash
...
# 处理前800行
head -n 800 /afs/cern.ch/work/z/zuhao/public/yanzx/isotpes_code/submit/joblist/list_MC_O16.txt | while read -r line; do
...
# 创建第二个脚本 process_last800.sh
cat > process_last800.sh << 'EOF'
#!/bin/bash
...
# 添加执行权限
chmod +x process_first800.sh process_last800.sh

# 运行命令：
# nohup ./process_first800.sh > mco16_process_first800.log 2>&1 &
# nohup ./process_last800.sh > mco16_process_last800.log 2>&1 &