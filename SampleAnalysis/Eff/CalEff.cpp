...
// Include Tool.h for SplineFit usage
#include "../Tool.h" 
...
void CalEff() {
    ...
    string outRatioFile = dirOut + "ISSMCRatioFit.root";
    string fileISS = dirIn + "HeToOxyEff.root";
    ...
    isoConfigs[4] = {{"Be7", "Be7_rew_frag4_withBkg_bkgest_eff.root", 0.6}, {"Be9", "Be9_rew_frag4_withBkg_bkgest_eff.root", 0.3}, {"Be10", "Be10_rew_frag4_withBkg_bkgest_eff.root", 0.1}};
    isoConfigs[5] = {{"B10", "B10_rew_frag4_withBkg_bkgest_eff.root", 0.3},{"B11", "B11_rew_frag4_withBkg_bkgest_eff.root", 0.7}};
    isoConfigs[6] = {{"C12", "C12_rew_frag4_withBkg_bkgest_eff.root", 1.0}};
    isoConfigs[7] = {{"N14", "N14_rew_frag4_withBkg_bkgest_eff.root", 0.5}, {"N15", "N15_rew_frag4_withBkg_bkgest_eff.root", 0.5}};
    isoConfigs[8] = {{"O16", "O16_rew_frag4_withBkg_bkgest_eff.root", 1.0}};
    ...
    for (int z = 4; z <= 8; ++z) {
        ...
        for (auto& task : tasks) {
            ...
            // --- 1. MC Mixing & Single Isotope Calc ---
            ...
            for (size_t i = 0; i < isoConfigs[z].size(); ++i) {
                ...
                // Calculate Single Isotope Efficiency
                ...
                if (cut == "Trigger") {
                    ...
                    cout<<hEffSingle->GetBinContent(5)<<" "<<hEffSingle->GetBinContent(6)<<" "<<hEffSingle->GetBinContent(7)<<endl;
            ...
            if (cut == "BkgReduction" && (z == 4 || z == 5 || z == 7)) {
                int zL = (z < 6) ? 2 : 6; int zH = (z < 6) ? 6 : 8;
                double alpha = (double)(z - zL) / (zH - zL);
                TH1D *hNL = (TH1D*)fISS->Get(("Eff_FLUXH1_BkgReduction_Num_" + det + "_Z" + to_string(zL)).c_str());
                TH1D *hDL = (TH1D*)fISS->Get(("Eff_FLUXH1_BkgReduction_Den_" + det + "_Z" + to_string(zL)).c_str());
                TH1D *hNH = (TH1D*)fISS->Get(("Eff_FLUXH1_BkgReduction_Num_" + det + "_Z" + to_string(zH)).c_str());
                TH1D *hDH = (TH1D*)fISS->Get(("Eff_FLUXH1_BkgReduction_Den_" + det + "_Z" + to_string(zH)).c_str());
                if (hNL && hNH) {
                    TH1D *eL = (TH1D*)hNL->Clone(); eL->Divide(hNL, hDL, 1, 1, "B");
                    TH1D *eH = (TH1D*)hNH->Clone(); eH->Divide(hNH, hDH, 1, 1, "B");
                    hEffISS = (TH1D*)eL->Clone(); hEffISS->Reset();
                    for(int i=1; i<=eL->GetNbinsX(); ++i) {
                        double vL = eL->GetBinContent(i), vH = eH->GetBinContent(i);
                        if(vL > 0 && vH > 0) {
                            hEffISS->SetBinContent(i, vL + alpha*(vH - vL));
                            hEffISS->SetBinError(i, hypot((1-alpha)*eL->GetBinError(i), alpha*eH->GetBinError(i)));
                        }
                    }
                    delete eL; delete eH;
                }
            ...
            if (!hNumTot || !hEffISS) continue;

            // --- 3. Mixed MC Efficiency & Ratio ---
            ...
            TH1D* hRatio = (TH1D*)hEffISS->Clone(Form("Ratio_Z%d_%s_%s", z, cut.c_str(), det.c_str()));
            ...
            double fMin = drawXMin, fMax = (cut == "InnerTracking") ? 19.5 : drawXMax;
            TF1* fit = RunAdaptiveSplineFit(hRatio, Form("fit_Z%d_%s_%s", z, cut.c_str(), det.c_str()), fMin, fMax, "b2e1");
            if (fit) {
                if (cut == "InnerTracking") {
                    double yVal = fit->Eval(19.5);
                    TF1* comp = new TF1(Form("fit_Z%d_%s_%s", z, cut.c_str(), det.c_str()), [fit, yVal](double* x, double* p){ return (x[0] < 19.5) ? fit->Eval(x[0]) : yVal; }, fMin, drawXMax, 0);
