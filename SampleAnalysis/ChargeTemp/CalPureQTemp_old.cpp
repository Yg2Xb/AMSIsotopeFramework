...
const std::string inputFileName = "/eos/user/z/zixuan/Isotope/Add/Be_frag4_withBkg_Tune_bkghist.root";
...
PurgeResult TemplatePurger::runFitAndPurge(
	...
	if (result.fitStatus <= 1) {
		...
		// ======= 新增功能：寻找峰值并从高Z方向截断低含量Bin =======
		int maxBin = result.purifiedHist->GetMaximumBin();
		int nBins = result.purifiedHist->GetNbinsX();
		bool triggerCut = false;

		// 从最大值的 bin 开始往高 Charge 方向遍历
		for (int i = maxBin; i <= nBins; ++i) {
			if (triggerCut) {
				result.purifiedHist->SetBinContent(i, 0.0);
				result.purifiedHist->SetBinError(i, 0.0);
				continue;
			}

			// 发现 content < 0.1，触发截断
			if (result.purifiedHist->GetBinContent(i) < 0.1) {
				result.purifiedHist->SetBinContent(i, 0.0);
				result.purifiedHist->SetBinError(i, 0.0);
				triggerCut = true; // 标记此后所有 bin 都清零
			}
		}
		// ======================================================
	...
	if (pullGraph) {
		...
		pullGraph->GetXaxis()->SetRangeUser(fitMin_, fitMax_);
...
void CalPureQTemp() {
	...
	for (const auto& detector : detectors) {
		...
		for (const auto& el : requiredElements) {
			...
			string nameInner = getTemplateHistName(el, detector, "InnerQTemplate"); // 新增

			TH2F* h2dL1_raw = (TH2F*)inputFile->Get(nameL1.c_str());
			TH2F* h2dL2_raw = (TH2F*)inputFile->Get(nameL2.c_str());
			TH2F* h2dInner_raw = (TH2F*)inputFile->Get(nameInner.c_str()); // 新增
			...
			if (h2dInner_raw) {
				h2d_raw_Inner[el] = unique_ptr<TH2F>((TH2F*)h2dInner_raw->Clone(Form("Inner_RAW_%s_%s", el.c_str(), detector.c_str())));
				h2d_raw_Inner[el]->RebinX(2);
			}
		}
		...
		// Save both L2 and Inner templates in one loop
		for (const auto& el : requiredElements) {
			outputFile->cd();

			// 1. Save L2 Templates (Only for Li, Be, B, C, N as per original logic)
			if ((el == "Lithium" || el == "Beryllium" || el == "Boron" || el == "Carbon" || el == "Nitrogen") && h2d_raw_L2.count(el)) {
				const auto& h_base = h2d_raw_L2.at(el);
				std::string name = Form("h2d_TuneQTemp_%s_%s", el.c_str(), detector.c_str());

				auto h2d = std::make_unique<TH2D>(name.c_str(), h_base->GetTitle(), 
						h_base->GetNbinsX(), h_base->GetXaxis()->GetXmin(), h_base->GetXaxis()->GetXmax(),
						h_base->GetNbinsY(), h_base->GetYaxis()->GetXmin(), h_base->GetYaxis()->GetXmax());
				h2d->GetYaxis()->Set(h_base->GetNbinsY(), h_base->GetYaxis()->GetXbins()->GetArray());

				for(int x=1; x<=h_base->GetNbinsX(); ++x) {
					for(int y=1; y<=h_base->GetNbinsY(); ++y) {
						h2d->SetBinContent(x, y, h_base->GetBinContent(x, y));
						h2d->SetBinError(x, y, h_base->GetBinError(x, y));
					}
				}
				h2d->Write();
			}

			// 2. Save InnerQTemplates (For all elements)
			if (h2d_raw_Inner.count(el)) {
				const auto& h_base = h2d_raw_Inner.at(el);
				std::string name = Form("h2d_InnerQTemp_%s_%s", el.c_str(), detector.c_str());
