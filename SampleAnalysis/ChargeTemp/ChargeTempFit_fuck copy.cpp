...
// --- UnifiedChargeFitter 类保持不变，但 fitMin_/fitMax_ 已修改 ---
...
bool UnifiedChargeFitter::initializeAndProject() {
    ...
    for (const auto& el : templateElements_) {
        ...
        if (h_templates_slices[el]) {
             //h_templates_slices[el]->Smooth(1,"G");
        }
        // ------------------------------------------
    ...
    // 设置拟合参数：Li, Be, B, C, N为自由参数，O为约束参数。拟合比例（非扩展）
    // <<< MODIFIED: 自由参数增加 Lithium
...
std::unique_ptr<RooPlot> UnifiedChargeFitter::generatePlotAndCalcChi2(UnifiedFitResult& result) {
    ...
    // <<< MODIFIED: 增加 Lithium 的颜色（排在第一个）
    std::vector<int> colors = {kBlack, kAzure + 7, kOrange - 3, kGreen + 2, kMagenta - 3, kCyan + 2};
...
// --- runUnifiedChargeAnalysis 函数 (已修改：增加 Lithium, 且模板名称模式不变) ---
...
void runUnifiedChargeAnalysis(const std::string& chain) {
    ...
    for (const auto& detector : detectors) {
        ...
        for (int y_bin = 1; y_bin <= n_bins_y; ++y_bin) {
            ...
            frame->SetMaximum(2 * hmax->GetMaximum());
            frame->Draw();
            ...
            // 示例：显示 Li 和 Be 在 Be 窗口的产额 (已包含 Li)
            double yield_Be_in_Be = result.yields_in_window["Boron"]["Beryllium"];
            double err_Be_in_Be = result.yields_in_window_err["Boron"]["Beryllium"];
            double yield_Li_in_Be = result.yields_in_window["Boron"]["Carbon"];
            double err_Li_in_Be = result.yields_in_window_err["Boron"]["Carbon"];
            info->AddText(Form("In Be window: N_{Li}=%.1f#pm%.1f, N_{Be}=%.1f#pm%.1f", yield_Li_in_Be, err_Li_in_Be, yield_Be_in_Be, err_Be_in_Be));
            info->Draw();
