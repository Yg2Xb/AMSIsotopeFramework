...
void ChargeFitProcessor::configure() {
    ...
    fitMin_ = z_frag - 0.2;
    fitMax_ = (sourceName_ == "Oxygen") ? z_source + 0.5 : z_source + 1.5;
...
bool ChargeFitProcessor::runFit() {
    ...
    constexpr double MAX_CHI2NDF = 10.0;
    ...
    while (currentFitMax >= minFitMax) {
        ...
        if (fitResult_ && fitResult_->status() == 0 && fitResult_->covQual() == 3 && std::isfinite(fitResult_->minNll())) {
            // Create a temporary frame to calculate Chi2 using the trusted external function
            ...
            if (chi2ndf < MAX_CHI2NDF) {
                ...
                fitMax_ = currentFitMax; // CRITICAL: Update class state with the successful range
                return true;
            } else {
...
FitResult ChargeFitProcessor::calculateFinalYield() {
    ...
    // Set the variable back to its full original range for any other potential use
...
std::unique_ptr<RooPlot> ChargeFitProcessor::generatePlotAndCalcChi2(FitResult& result) {
    ...
    for(int i = 1; i <= h_signal_extended_->GetNbinsX(); ++i) {
        double binCenter = h_signal_extended_->GetBinCenter(i);
        // Use the final, possibly shrunken, fit range for NDF calculation
    ...
    // Use the trusted method from Tool.h for the final reported value
