...
// ======================= ROBUST MODIFICATION 1: FUNCTION-BASED INITIALIZATION =======================
// A structure to hold special configurations for certain source elements.
struct SpecialFitConfig {
    std::vector<std::string> templateElements;
    std::pair<double, double> fitRange;
};

// To avoid complex global initialization issues with ROOT's interpreter (Cling),
// we initialize the configuration map inside a function.
const std::map<std::string, SpecialFitConfig>& get_special_configs() {
    // This static map is initialized only once, the first time this function is called.
    static std::map<std::string, SpecialFitConfig> configs;
    if (configs.empty()) {
        // Use simple, step-by-step insertion, which is much safer for Cling.
        configs["Oxygen"] = {
            /* templateElements */ { "Carbon", "Nitrogen", "Oxygen" },
            /* fitRange         */ { 5.6, 8.5 }
        };
...
{
    // Before creating ROOT objects, ensure ROOT's automatic memory management is disabled for them.
    // This prevents double-free errors when using smart pointers.
...
void ChargeFitProcessor::configure() {
    ...
    // ======================= ROBUST MODIFICATION 2: USE THE GETTER FUNCTION =======================
    const auto& special_configs = get_special_configs();
    auto it_cfg = special_configs.find(sourceName_);
    // =================================== END OF MODIFICATION 2 ====================================
    
    if (it_cfg != special_configs.end()) {
        const auto& config = it_cfg->second;
        elements_to_fit.insert(config.templateElements.begin(), config.templateElements.end());
        fitMin_ = config.fitRange.first;
        fitMax_ = config.fitRange.second;
    } else {
        ...
        std::vector<std::string> temp_elements(elements_to_fit.begin(), elements_to_fit.end());
        std::sort(temp_elements.begin(), temp_elements.end(), [&](const std::string& a, const std::string& b) {
            return element_db.at(a).Z < element_db.at(b).Z;
        });

        if (!temp_elements.empty()) {
            int min_z = element_db.at(temp_elements.front()).Z;
            int max_z = element_db.at(temp_elements.back()).Z;
            fitMin_ = min_z - 0.3;
            fitMax_ = max_z + 0.3;
        } else {
            fitMin_ = z_source - 1.3;
            fitMax_ = z_source + 1.3;
        }
    }
...
std::unique_ptr<TH1D> ChargeFitProcessor::projectSlice(TH2F* h2d, const char* name) {
    // TH1::AddDirectory(kFALSE) is now global, so this is safe.
...
bool ChargeFitProcessor::initializeAndProject() {
    ...
    for (const auto& el : free_params_elements) {
        double initial_val = 0.3;
        if (el == sourceName_) initial_val = 0.5;
        if (should_fit_fragment_ && el == fragmentName_) initial_val = 0.3;
...
bool ChargeFitProcessor::runFit() {
    ...
    constexpr double MAX_CHI2NDF = 1000.0;
    const double minFitMax = element_db.at(sourceName_).Z + 0.3;
...
FitResult ChargeFitProcessor::calculateFinalYield() {
    const bool debug_output = false; // Set to false for production
    FitResult res;
...
void runFragmentationAnalysis(const std::string& source, const std::string& fragment, const std::string& chain) {
    ...
    if (fragment == "Beryllium") currentInputFileName = "/eos/user/z/zixuan/Isotope/Add_NoB/Be_frag4.root";
    ...
    for (const auto& detector : detectors) {
        ...
        // ======================= ROBUST MODIFICATION 3: USE THE GETTER FUNCTION =======================
        const auto& special_configs = get_special_configs();
        auto it_cfg = special_configs.find(source);
        // =================================== END OF MODIFICATION 3 ====================================
        if (it_cfg != special_configs.end()) {
            const auto& templates = it_cfg->second.templateElements;
            elements_to_load_set.insert(templates.begin(), templates.end());
        } else {
        ...
        for (const auto& el : required_elements) {
            ...
            if ((source == "Oxygen" && el == "Carbon") || (source == "Nitrogen" && el == "Carbon")) {
                template_type = "L2QTemplate";
            } 
            // --- GENERIC LOGIC FOR ALL OTHER CASES ---
            else {
            ...
            templates_rebinned[el]->RebinX(2);
        }
        ...
        for (int y_bin = 1; y_bin <= n_bins_y; ++y_bin) {
            ...
            c_pdf->Clear();
            if (result.fit_status != 0 && result.fit_status != -1) {
            ...
            auto info = std::make_unique<TPaveText>(0.15, 0.74, 0.6, 0.86, "NDC");
            info->SetFillStyle(0); info->SetBorderSize(0); info->SetTextAlign(12);
            info->SetTextSize(0.035);

            if(result.fit_status == 0) {
            // --- 🎯 目标修改区域：构建第一行文本 (Chi2 + all P_Z fractions) ---
            
            // 1. 构建第一行文本 (Chi2/NDF + Narrow Fractions)
            std::string line1 = Form("#chi^{2}/NDF = %.2f, L1Q [sourceZ-0.2, sourceZ+0.4]: ", result.chi2ndf);
            for(const auto& el : processor.getTemplateElements()) {
                line1 += Form("frac_{%s}=%.3f, ", el.substr(0,3).c_str(), result.narrow_fractions.at(el));
            }
            // 移除末尾多余的 ", "
            if (line1.size() > 2) line1.resize(line1.size() - 2); 
            info->AddText(line1.c_str());

            // 2. 构建第二行文本 (N_sig + two Yields)
            std::string line2 = Form("N_{sig} =%.1f, ", result.N_sig_narrow);
            line2 += Form("N_%s=%.2f #pm %.2f, ", fragment.c_str(), result.fragment_yield, result.fragment_yield_err);
            line2 += Form("N_%s=%.2f #pm %.2f", source.c_str(), result.source_yield, result.source_yield_err);
            info->AddText(line2.c_str());

            } else {
            // 拟合失败的默认信息
            info->AddText("Fit Skipped/Failed:");
            info->AddText(Form("Fit Status = %d", result.fit_status)); // 显示状态码帮助调试
            }
            info->Draw();
...
void ChargeTempFit_old() {
    ...
    runFragmentationAnalysis("Boron", "Beryllium", "UnbiasedL1Inner");
    runFragmentationAnalysis("Carbon", "Beryllium", "UnbiasedL1Inner");
    runFragmentationAnalysis("Nitrogen", "Beryllium", "UnbiasedL1Inner");
    runFragmentationAnalysis("Oxygen", "Beryllium", "UnbiasedL1Inner");
    //runFragmentationAnalysis("Carbon", "Boron", "UnbiasedL1Inner");
    //runFragmentationAnalysis("Nitrogen", "Boron", "UnbiasedL1Inner");
    //runFragmentationAnalysis("Oxygen", "Boron", "UnbiasedL1Inner");
}
