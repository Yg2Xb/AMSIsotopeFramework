...
void plotFitFileVersionComparison(TFile* fin_v05, TFile* fin_v08, TCanvas* c, const string& pdfName) {
    cout << "\n[PROCESS] Starting Fit File Version Comparison plots..." << endl;
    for (const auto& element : gConfig.elements) for (const auto& chain : gConfig.chains) for (const auto& det : gConfig.detectors)
    for (const auto& temp : gConfig.templates) for (const auto& param : gConfig.paramList) {
        string name = chain+"_"+element+"_"+det+"_"+temp+"_"+param.first+"_"+param.second;
        unique_ptr<TH1> h1(getHist(fin_v05, name)), h2(getHist(fin_v08, name));
        if (!isValidHist(h1.get()) && !isValidHist(h2.get())) continue;
        string title = element+" "+chain+" "+det+" "+temp+" "+param.first+"_"+param.second;
        drawPage(c, {h1.get(), h2.get()}, {gConfig.versionColors["v05"], gConfig.versionColors["v08"]},
                 gConfig.versionLabels, title, param.second,
                 gConfig.detRanges[det].first, gConfig.detRanges[det].second, pdfName);
    }
}
...
void plotTemplateComparison(TFile* fin, TCanvas* c, const string& pdfName) {
    ...
    for (const auto& param : gConfig.paramList) {
        ...
        if (param.second == "AlphaL") {
            const double alpha_const_val = h->GetMaximum(); 
            string fit_name = name + "_spline";
            TF1* const_fit = new TF1(fit_name.c_str(), Form("%f", alpha_const_val), 0.25, 22);
            if (gSplineOutFile && gSplineOutFile->IsOpen()) {
                gSplineOutFile->cd();
                const_fit->Write();
                cout << "  [SUCCESS] Wrote constant TF1 '" << fit_name << "' to file." << endl;
            }
            ...
            labels.push_back(temp == "L1QTemplate" ? "L1Temp" : "L2Temp");
...
void compareFitResults() {
    ...
    string pdfName_version = gConfig.output_dir + "FitParam_VersionCompare.pdf";
