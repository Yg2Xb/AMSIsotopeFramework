...
#include <vector>
#include <map>
#include "/afs/cern.ch/work/z/zuhao/public/yanzx/IsoAnalys/acc/RootFunc.h"
...
namespace Constants {
    ...
    const int EFF_RIGIDITY_BINS = 40;
    const int BETA_TYPES = 3;
    const int TRACKER_CUTS = 5;
    const double SAFE_FACTOR_RIG = 1.2;
}
...
public:
    ...
    int isotope_count_;
    std::string name_;
    std::array<int, Constants::MAX_ISOTOPES> mass_;
    std::array<int, Constants::MAX_ISOTOPES> color_;

    IsotopeVar(int charge, int num, std::string name, 
               std::array<int, Constants::MAX_ISOTOPES> mass,
               std::array<int, Constants::MAX_ISOTOPES> color)
        : charge_(charge)
        , isotope_count_(num)
        , name_(std::move(name))
        , mass_(mass)
...
public:
    ...
    double beta_lowest_;
    std::array<double, 2> Ekn_range_;  // 修改为double类型以保持一致性
    int color;

    BetaConfig(std::string name, double safety_factor,
               double beta_lowest, std::array<double, 2> Ekn_range, int color)
        : name_(std::move(name))
        , safety_factor_(safety_factor)
        , beta_lowest_(beta_lowest)
        , Ekn_range_(Ekn_range)
        , color(color)
    {}
};
...
const std::array<IsotopeVar, Constants::ELEMENT_COUNT> IsotopeData {{
    IsotopeVar(1, 2, "Proton",  std::array<int,3>{{1, 2, 0}},   std::array<int,3>{{kBlue, kGreen+2, kOrange+1}}),
    IsotopeVar(2, 2, "Helium",  std::array<int,3>{{3, 4, 0}},   std::array<int,3>{{kBlue, kGreen+2, kOrange+1}}),
    IsotopeVar(3, 2, "Lithium", std::array<int,3>{{6, 7, 0}},   std::array<int,3>{{kBlue, kGreen+2, kOrange+1}}),
    IsotopeVar(4, 3, "Beryllium", std::array<int,3>{{7, 9, 10}},  std::array<int,3>{{kBlue, kOrange+1, kGreen+2}}),
    IsotopeVar(5, 2, "Boron",   std::array<int,3>{{10, 11, 0}}, std::array<int,3>{{kBlue, kGreen+2, kOrange+1}}),
    IsotopeVar(6, 2, "Carbon",  std::array<int,3>{{12, 13, 0}}, std::array<int,3>{{kBlue, kGreen+2, kOrange+1}}),
    IsotopeVar(7, 2, "Nitrogen",std::array<int,3>{{14, 15, 0}}, std::array<int,3>{{kBlue, kGreen+2, kOrange+1}}),
    IsotopeVar(8, 3, "Oxygen",  std::array<int,3>{{16, 17, 18}}, std::array<int,3>{{kBlue, kOrange+1, kGreen+2}})
}};
} // anonymous namespace
...
namespace Detector {
    const std::array<BetaConfig, Constants::BETA_TYPES> BetaTypes {{
        BetaConfig("TOF",  1.06,   0.5,   {{0.4, 1.5}}, kBlue),
        BetaConfig("NaF",  1.005,  0.75,  {{1.0, 4.0}}, kOrange+1),
        BetaConfig("AGL", 1.0005, 0.953, {{2.5, 17.5}}, kGreen+2)
    }};
    ...
    const std::array<std::string, 10> TOFCuts {{
        "PhysTrig", "+BasicAndFiducial", "+InnerTrk", "+InnerQ", 
        "+BigZ", "+UTOFQ", "+BG_Twiki", "+TOFGeoExS3S4", "+tofChi2", "No More cut"
    }};

    const std::array<std::string, 10> RICHCuts {{
        "PhysTrig", "+BasicAndFiducial", "+InnerTrk", "+InnerQ",
        "+BigZ", "+UTOFQ", "+BG_Twiki", 
        "+richGeo", "+richRec", "+richCharge"
    }};
...
namespace Acc {
    ...
    double TofEdgeEk = betaToKineticEnergy(0.5);
    double NaFEdgeEk = betaToKineticEnergy(0.75);
    double AGLEdgeEk = betaToKineticEnergy(0.953);
    ...
    std::array<std::vector<double>, 3> xpointsEkfor7 = {{{0.25, 0.4, 0.63, 0.89, 1.41, 2.00, 3.16, 5.62, 10.00, 30, 60, 100, 300, 650},
                                                    {1.4 * NaFEdgeEk, 0.75, 0.92, 1.25, 1.41, 2.00, 3.16, 5.62, 10.00, 30, 60, 100, 300, 650},
                                                    {1.1 * AGLEdgeEk, 2.3, 3.2, 3.8, 4.5, 5.62, 10.00, 30, 60, 100, 300, 650}}};
    std::array<std::vector<double>, 3> xpointsEkfor9 = {{{0.235, 0.3, 0.4, 0.63, 0.89, 1.41, 2.00, 3.16, 5.62, 10.00, 30, 60, 100, 300, 650},
                                                    {1.3 * NaFEdgeEk, 0.75, 0.92, 1.25, 1.41, 2.00, 3.16, 5.62, 10.00, 30, 60, 100, 300, 650},
                                                    {1.1 * AGLEdgeEk, 2.3, 3.2, 3.8, 4.5, 5.62, 10.00, 30, 60, 100, 300, 650}}};
    std::array<std::vector<double>, 3> xpointsEkfor10 = {{{0.22, 0.31, 0.4, 0.63, 0.89, 1.41, 2.00, 3.16, 5.62, 10.00, 30, 60, 100, 300, 650},
                                                    {1.2 * NaFEdgeEk, 0.75, 0.92, 1.25, 1.41, 2.00, 3.16, 5.62, 10.00, 30, 60, 100, 300, 650},
                                                    {1.1 * AGLEdgeEk, 2.3, 3.2, 3.8, 4.5, 5.62, 10.00, 30, 60, 100, 300, 650}}};
    // Style settings
    void acc_goodlogon() {
...
namespace FluxAnalys {
    std::vector<double> xpointsEkTOF = {0.6, 0.9, 1.5};
    std::vector<double> xpointsEkNaF = {0.6, 0.9, 1.5, 2.6, 4.5};
    std::vector<double> xpointsEkAGL = {2.7, 3.4, 4.6, 5.8, 7.0, 8.8, 10.3, 16};
    std::vector<double> xpointsEkcombine = {0.4, 0.7, 1.2, 2.5, 8.0, 16.3};
    
    std::vector<double> xpointsEkBe = {0.7, 0.85, 1.4, 1.9, 3.1, 5, 9, 12};
}
