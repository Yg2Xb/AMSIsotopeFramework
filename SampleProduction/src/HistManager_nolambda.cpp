...
std::vector<std::string> HistManager::GetActiveBranches() {
    return {
        // RTI
        "run", "isreal", "event", "time", "zenith", "issaa", "glat", "glong",
        "thetam", "phim", "mcutoffi", "rtilf", "rtinev", "rtinerr", "rtintrig",
        "rtinpar", "rtigood", "rtinexl", "irti",
        
        // Tracker
        "physbpatt2", "itrtrack", "ntrack", "tk_hitb", "tk_qin", "tk_qrmn", "tk_qln", "tk_qls",
        "tk_exqln", "tk_exqls", "tk_rigidity1", "tk_chis1", "tk_pos", "tk_dir",
        "tk_pos1s", "betah2hb", "betah2r",
        ...
        // RICH
        "rich_beta", "rich_pb", "rich_pmt", "rich_npe", "rich_good", "rich_clean",
...
HistManager::HistManager(const std::string& output_filename,
	...
	const std::vector<int> FragA = (fragZ == 4) ? std::vector<int>{7, 9, 10} : std::vector<int>{10, 11};
	...
	//auto ekBinsStd = safeBins("old_EkPerNucleon");
	...
	for (int c = 0; c < Nchain; ++c) {
		...
		IDH5a2[c] = createHist<TH2D>(
				Form("%s_ID_H5a2", chains[c].c_str()),
				"NaF-Tracker #Delta(1/#beta);NaF-Tracker #Delta(1/#beta);gene Rigidity [GV]",
				400, -0.2, 0.2, static_cast<int>(rigBins.size()) - 1, rigBins.data());
		IDH5a3[c] = createHist<TH2D>(
				Form("%s_ID_H5a3", chains[c].c_str()),
				"NaF-true #Delta(1/#beta);NaF-True #Delta(1/#beta);gene Rigidity [GV]",
				400, -0.2, 0.2, static_cast<int>(rigBins.size()) - 1, rigBins.data());
		IDH5b[c] = createHist<TH2D>(
				Form("%s_ID_H5b2", chains[c].c_str()),
				"AGL-Tracker #Delta(1/#beta);AGL-Tracker #Delta(1/#beta);Rigidity [GV]",
				400, -0.2, 0.2, static_cast<int>(rigBins.size()) - 1, rigBins.data());
		IDH5b2[c] = createHist<TH2D>(
				Form("%s_ID_H5b3", chains[c].c_str()),
				"AGL-Tracker #Delta(1/#beta);AGL-Tracker #Delta(1/#beta);gene Rigidity [GV]",
				400, -0.2, 0.2, static_cast<int>(rigBins.size()) - 1, rigBins.data());
		IDH5b3[c] = createHist<TH2D>(
				Form("%s_ID_H5b", chains[c].c_str()),
				"AGL-True #Delta(1/#beta);AGL-True #Delta(1/#beta);gene Rigidity [GV]",
				400, -0.2, 0.2, static_cast<int>(rigBins.size()) - 1, rigBins.data());
		IDH6a[c] = createHist<TH2D>(
	...
	if (isISS) {
				...
				for (int d = 0; d < Ndet; ++d) {
					...
					for (int t = 0; t < Nct; ++t) {
						...
						ISS_BKGH2[c][s][d][t] = createHist<TH2D>(
								...
								//x_bins, x_min, x_max, static_cast<int>(rigBins.size()) - 1, rigBins.data());
					}
		...
		createMcBkgHists(MC_BKGH2b, "H2b", "%s %s MC L1Orig-L2toL4 frag Isotope Mass%d Counts vs E_{k}/n;%s E_{k}/n [GeV/n];Counts");
		createMcBkgHists(MC_BKGH3a, "H3a", "%s %s MC L2 frag Isotope Mass%d Counts vs E_{k}/n;L2 Truth E_{k}/n [GeV/n];Counts");
		createMcBkgHists(MC_BKGH3b, "H3b", "%s %s MC L2 frag Isotope Mass%d Counts vs E_{k}/n;Measured E_{k}/n [GeV/n];Counts");
		createMcBkgHists(MC_BKGH3c, "H3c", "%s %s MC L2toL4 frag Isotope Mass%d Counts vs E_{k}/n;Measured E_{k}/n [GeV/n];Counts");
	}
	...
	if (isISS) {
		...
		MC_FLUXH3[0] = createHist<TH1D>(//std bin for bkg analysis
				"MC_FLUX_H3",
...
void HistManager::PrepareFilteredTree(TChain* dataChain) {
    ...
    m_filteredTree->SetAutoSave(0);  // 禁用自动保存，在 Save() 中统一写入
...
void HistManager::Save(bool saveTree) {
    ...
    // 保存所有直方图
    std::cout << "HistManager: Saving histograms..." << std::endl;
    ...
    if (saveTree && m_filteredTree) {
        std::cout << "HistManager: Writing filtered TTree with " 
                  << m_filteredTree->GetEntries() << " events..." << std::endl;
        m_outputFile->cd();
        m_filteredTree->Write("", TObject::kOverwrite);
        std::cout << "HistManager: Filtered TTree saved successfully." << std::endl;
    } else if (saveTree) {
        std::cout << "HistManager: Warning: No filtered tree to save (was PrepareFilteredTree called?)." << std::endl;
    }

    // ==================== 关闭文件 ====================
    std::cout << "HistManager: Closing output file." << std::endl;
    m_outputFile->Close();
}