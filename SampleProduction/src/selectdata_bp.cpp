...
void selectdata::Loop() {
	...
	for (Long64_t jentry = 0; jentry < nentries; ++jentry) {
		...
		// all event in filter tree has good RTI
		...
		*/
		...
		bool detValidBkg[3] = { //charge-independent
			tof_cut.cutTOFforBkg(charge, isISS).details[0], //no chi2 cut, only geo cut
		...
		*/
		...
		//fill tree
		if (filteredTree) {
			bool L1Norm_Need = l1n.details[2] && l1n.details[3] && l1n.details[4] && tk_ql1 > 2.4 && tk_ql1 < 8.8; 
    		bool L1Unb_Need  = l1u.details[2] && l1u.details[3] && tk_ql1_unbiased > 2.4 && tk_ql1_unbiased < 8.8; 
		...
		for (int iz = 0; iz < nZ; ++iz) {
			...
			for (int c = 0; c < std::min(2, NchainLoc); ++c) {
				...
				if (rig_chain[1] > 0 && AveMass > 0 && RecCharge_Int > 0) {
					beta_tracker = isISS ? Tools::rigidityToBeta(rig_chain[1], z_use, AveMass, false) : Tools::rigidityToBeta(rig_chain[1], charge, UseMass, false);
				}
					...
					if (beta_det[0] > 0 && richBeta > 0) {
						const double dx = 1.0 / beta_det[0] - 1.0 / richBeta;
							if (BetaDetectorCutResult_use[1] && getBeyondBetaCutoffCut(1, z_use, A_use)) {
								histManager->IDH6a[iz][c].get()->Fill(dx, beta_det[1], weight_NucFlux);
							...
							if (BetaDetectorCutResult_use[2] && getBeyondBetaCutoffCut(2, z_use, A_use)) {
								histManager->IDH6b[iz][c].get()->Fill(dx, beta_det[2], weight_NucFlux);
					...
					for (int d = 0; d < NdetLoc; ++d) {
						...
						// t = 2 assumed to be L2QTemplate with L2Q Tuning
						{
							...
							if (pass && BetaDetectorCutResult_zsrc[d]) { // L2QTemplate uses FULL detector quality
								...
								double used_ql2 = s < 1 ? tk_ql2 : Tools::tuneL2Charge(
									current_chain, current_nucleus, current_detector, ekBin, tk_ql2
								);
								// 4. Fill the histogram with the TUNED L2 charge.
								histManager->ISS_BKGH2[c][s][d][2].get()->Fill(used_ql2, ek_det[d], 1.0);
								// --- End of L2Q Tuning ---
							}
						}
					}
