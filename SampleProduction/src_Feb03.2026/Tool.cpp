...
double calculateWeight(double mmom, int charge, double mass, bool isISS) {
    if (isISS || charge == 0) return isISS ? 1.0 : 0.0;

    const double geneRig = std::abs(mmom) / std::abs(charge);
    if (geneRig < geneRig_low || geneRig > geneRig_up) return 0.0;

    if (!(MC_norm > 0)) return 0.0;
    const double mc_val = f_MC.Eval(geneRig) / MC_norm;
    ...
    double flux_val = 0.0;
    if (name.empty() || itF == fluxMap.end() || itN == fluxNorm.end()) {
        if (!(Reweight_norm > 0)) return 0.0;
        flux_val = f_Reweight.Eval(geneRig) / Reweight_norm;
    } else {
        if (!(itN->second > 0)) return 0.0;
        flux_val = itF->second->Eval(geneRig) / itN->second;
    }
