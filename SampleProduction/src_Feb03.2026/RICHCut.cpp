...
CutResult<6> RICHCut::cutGeometry(bool interpolate, int mode) const {
    auto pos = getModifiedPosition(interpolate);
    
    bool treatAsNaF = (mode == 1) || (mode == -1 && isNaF());
    ...
    std::array<bool, 6> cuts{
        ...
        std::max(std::abs(pos[0]), std::abs(pos[1])) <= RICH::aer_border[0] || 
        std::max(std::abs(pos[0]), std::abs(pos[1])) >= RICH::aer_border[1],
